...
int main(int argc, char **argv) {
    ...
    for(;;) {
        ...
        Mat gradX;
        Sobel(gray, gradX, ddepth, 1, 0, kSize, scale, delta);
        Mat gradY;
        Sobel(gray, gradY, ddepth, 0, 1, kSize, scale, delta);
        Mat absGradX, absGrandY;
        convertScaleAbs(gradX, absGradX);
        convertScaleAbs(gradY, absGrandY);
        addWeighted(absGradX, 0.5, absGrandY, 0.5, 0, grad);
        imshow("grad", grad);
